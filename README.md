...
当前仓库采用混合数据源，便于同时体验真实接入与原有演示流程：

- `demo-service` 通过 Prometheus HTTP API 查询真实请求速率、5xx 错误率和 P95 延迟。
- `order-service`、`payment-service` 等原有示例服务继续使用 mock 数据。
- 本地真实监控闭环的启动和验证方法见 [`demo-ops/README.md`](demo-ops/README.md)。

真实指标查询支持“最近15分钟”“最近1小时”“最近24小时”“今天”“本周”“本月”，可通过环境变量 `PROMETHEUS_BASE_URL` 覆盖默认地址 `http://localhost:9090`。告警、日志、拓扑和报告聚合目前仍为演示数据，后续可继续对接 Elasticsearch、告警平台、CMDB 或服务拓扑平台。
...
- 扩展 `demo-service` 的真实 Prometheus 接入到报告聚合流程
...
### Q2：为什么部分服务的数据看起来是固定的？

因为当前采用混合数据源：`demo-service` 已接入真实 Prometheus 指标，原有的 `order-service`、`payment-service` 等示例服务仍使用 mock 数据，以保留开箱即用的 Agent 演示流程。真实演示步骤见 [`demo-ops/README.md`](demo-ops/README.md)。
